...
%------------------------------------------------------------
\begin{abstract}
Shallow depth of field in macro and close-up photography prevents any single image from rendering the entire subject in focus. Focal-stack fusion addresses this by combining a sequence of images that each focus on a different depth plane into one all-in-focus composite. This work implements and evaluates the complete focal-stack pipeline—image loading, two-stage ORB-homography and ECC alignment, focus measure computation, focus map construction, and multi-scale fusion—on six datasets of increasing realism: a 23-frame plants baseline, a personal 6-frame flower macro stack, a 6-frame personal handheld stack, a 4-frame evening stack, a 3-frame insect stack captured in the field, and pre-registered Lytro two-focus pairs. Four focus measures (Laplacian variance, Tenengrad energy, local contrast, and Sum-Modified Laplacian) are compared systematically, and three fusion strategies (winner-takes-all, soft/softmax blending, and Laplacian pyramid fusion) are evaluated using Tenengrad sharpness, SSIM, and Shannon entropy. Tenengrad consistently ranks first across all datasets; WTA fusion achieves the highest sharpness gain while pyramid blending best preserves structural similarity.
\end{abstract}
...
\begin{itemize}
  ...
  \item A two-stage alignment strategy (ORB feature matching + homography, refined with ECC) that robustly handles focus breathing—the scale change induced by lens refocusing—which causes ghosting when translation-only ECC is used.
  \item Application of the pipeline to four personal stacks (flower macro, handheld, evening, and insect) captured with a smartphone.
...
For handheld stacks the camera moves between exposures, producing misregistration that creates ghosting in the fused output. An initial implementation using translation-only ECC~\cite{evangelidis2008parametric} was found to be insufficient because smartphone lenses exhibit \emph{focus breathing}: the effective focal length changes slightly as the lens refocuses, introducing a per-frame scale factor of up to 9\% between the nearest and farthest focus positions. Translation-only ECC cannot recover scale and converges to incorrect local minima when the displacement is large.

A two-stage alignment strategy was therefore adopted. In Stage~1, ORB keypoints~\cite{rublee2011orb} are detected in each frame and matched to the reference frame using a $k$-NN matcher with Lowe's ratio test ($r=0.75$). A homography is estimated from the inlier correspondences via RANSAC (reprojection threshold 3\,px). A sanity check rejects degenerate solutions where the implied rotation exceeds $30^\circ$ or the scale factor lies outside $[0.80,\,1.25]$; such cases fall back to a phase-correlation estimate initialised ECC translation. In Stage~2, ECC with 100 iterations and threshold $10^{-5}$ refines the sub-pixel residual translation on top of the Stage~1 warp. This pipeline eliminated the ghosting artefacts observed in the flower, insect, and evening stacks under translation-only alignment.
...
\textbf{Flower stack.} Six ProRAW frames ($f/1.78$, 6.765\,mm) of a tulip arrangement captured with an iPhone 17 Pro Max on 23 May 2026. Images were converted from HEIC to JPEG via \texttt{sips} with EXIF-rotation correction ($-$90\,CW), then downsampled to 1920\,px on the long edge. ORB + ECC alignment is applied. Focus breathing across the six focal planes introduced a scale variation of up to 9\%, which was not recoverable by translation-only ECC; the homography-based Stage~1 alignment eliminates this artefact.

\textbf{Personal handheld stack.} Six frames of an indoor scene captured hand-held with an iPhone 17 Pro Max. The stack tests the pipeline on a general scene with moderate depth variation. ORB + ECC alignment is applied.

\textbf{Evening stack.} Four ProRAW frames from an evening indoor session (same camera). The scene is a table with objects at multiple depths, captured in artificial light. This stack validates generalisation across lighting conditions. ORB + ECC alignment is applied.
...
\begin{enumerate}
  ...
  \item A two-stage ORB-homography + ECC alignment pipeline successfully mitigates both camera shake and focus breathing (lens scale change) in handheld stacks but cannot compensate for subject motion; pyramid blending is more robust than WTA in the subject-motion case (insect: SSIM 0.680 vs.\ 0.649).
